...
type Route struct {
	...
	OutlinkId    *Outlink    `orm:"column(outlink_id);rel(fk)"`
	NetlinksetId *Netlinkset `orm:"column(netlinkset_id);rel(fk)"`
	RoutesetId   *Routeset   `orm:"column(routeset_id);rel(fk)"`
	Enable       int8        `orm:"column(enable)"`
	Priority     int16       `orm:"column(priority)"`
	Score        int16       `orm:"column(score)"`
